...
def run_pipeline(
    ...
    strip_mask, strip_info = choose_strip_component(gray_blur, debug=debug)
    ...
    roi_files = sorted([str(p) for p in output_dir.glob("roi_*.png")])
    payload = {
        "output_dir": str(output_dir),
        "results_json": str(results_path),
        "step_01": str(output_dir / "step_01_input.png"),
        "step_04": str(output_dir / "step_04_strip_box.png"),
        "step_06": str(output_dir / "step_06_lanes_and_circles.png"),
        "roi_files": roi_files,
        "all_png_files": sorted([str(p) for p in output_dir.glob("*.png")]),
        "num_rois_saved": int(step67_result["num_rois_saved"]),
...
def main() -> None:
    ...
    print(f"Saved outputs to: {payload['output_dir']}")
    print(f"Saved results JSON: {payload['results_json']}")
    print(f"ROIs saved: {payload['num_rois_saved']} (num-strips hint {int(args.num_strips)})")
